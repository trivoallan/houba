...
def test_write_archive_refuses_a_non_canonical_arcname_even_without_the_planner(
    ...
    out = tmp_path / "a.zip"
    # Not "escapes": a non-canonical path plainly does not escape the root, and the two
    # rules are separate predicates precisely so the refusal can say which one fired.
    with pytest.raises(ArchiveError, match="non-canonical"):
