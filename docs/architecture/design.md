...
package-level **SBOM** (SPDX and/or CycloneDX) that standalone `syft` generates and attaches as an
OCI referrer.
...
flowchart TD
    ...
    domain["<b>domain/</b> — pure<br/>mirror_policy · selection · aliases · semver · expand · policy_merge · variants<br/>reconcile · collision · sharding · stamp · attestation · coverage · lifecycle · retention · purge · sbom · scan/* · transforms/*"]
...
- **`audit [--registry NAME] [--fail-on-uncovered] [--signed] [--fail-on-unsigned] [--sbom]`** —
  ...
  `with_sbom` / `without_sbom`) — observational only, no gate.
- **`gc [--registry NAME] [--keep N] [--older-than-days D] [--apply]`** — garbage-collect superseded
  scan-result referrers across the roster: keep the N newest per `(tool, format)` older than the grace
  window and collect the rest. The decision is purely temporal (no usage oracle). Dry-run by default;
  `--apply` deletes.
...
- **Delivered** — the full hexagon (`domain/`, `ports/`, `adapters/`, `use_cases/`, `cli/`); both
  ...
  reconcile; a `regctl` + `buildctl` + `cosign` + `syft` runtime image; and a kind-based
