...
from knock.domain.packaging import ArchiveEntry, path_escapes_root, path_is_canonical
...
def _write_entries(root: Path, entries: list[ArchiveEntry], tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(tmp_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for entry in entries:
            ...
            if not path_is_canonical(entry.path):
                # `plan_archive` normalises on the way in and stores the canonical form,
                # so anything arriving here non-canonical was hand-built and bypassed the
                # planner — the same reachability class as the escape and symlink checks.
                raise ArchiveError(f"refusing a non-canonical archive path: {entry.path}")
